...
export const defaultCategories:IDefaultCategory[] = [
  ...
  const data = [...defaultCategories] as const;
  export type Category = (typeof data[number])["id"];
  
  function validate(input: string): input is Category {
    return data.some((item) => item.id === input);
  }
